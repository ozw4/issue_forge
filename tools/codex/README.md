...
Each pending ID receives at most one sequential Fixer attempt in that cycle. Immediately before the invocation, the engine backs up and hashes the two pending artifacts, cumulative resolution, two active artifacts, and generated Fixer prompt. After every Fixer exit, including a non-zero exit, it compares all six. Any missing, replaced, or byte-modified scheduler artifact triggers an attempted per-file atomic restore to its pre-Fixer contents and stops the flow before the Fixer log is parsed or a claim is appended. If a target was replaced by a directory, or restoration or verification otherwise cannot complete, the engine reports the affected artifact and retains the backup directory for manual recovery. The Fixer chooses the smallest safe implementation, does not intentionally address another pending finding, starts no multi-agent or parallel work, and normally runs only the smallest directly relevant validation. Focused validation is not acceptance evidence and does not replace configured full Checks. Queue resume derives Batch Fixer progress only from run-owned attempts and cumulative resolution state; a successfully restored resolution leaves the mutating attempt unmatched, so resume fails closed before another Fixer starts.
