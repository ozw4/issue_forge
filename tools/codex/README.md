...
- queue control-plane coverage uses real `kill -9` owner death before worker registration, after registration, after authorization, after durable `run.state=completed`, and during a paused external child; it verifies parent-liveness self-release, registration/authorization fencing, cleanup-only completed finalization, exact terminal checkpoint phases, and exact Issue/Git/push/PR/merge counters
