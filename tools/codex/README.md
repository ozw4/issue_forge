...
- queue control-plane coverage uses real `kill -9` owner death before worker registration, after registration, after authorization, after durable `run.state=completed`, at every completed-cleanup transaction boundary, and during a paused external child; it verifies configuration-independent cleanup, parent-liveness self-release, registration/authorization fencing, cross-run isolation, exact terminal checkpoint phases, and byte-identical Issue/Codex/Git/push/PR/merge counters across finalization retries
