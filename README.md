...
必要なdetails artifactや対応行が欠落・重複・不整合の場合、flowは簡潔なfindingからdetailsを合成せず停止します。各pending IDはrejected-review cycleごとに最大1回だけFixerへ渡され、`fix-resolution.tsv` はそのcycleのclaimを処理順に累積します。各Fixerの前後ではpending 2 artifact、累積resolution、active 2 artifact、生成済みFixer promptがbyte-identicalであることを、Fixerの終了statusにかかわらず確認します。変更時は6 artifactをFixer前の内容へatomic replaceで復元し、resolutionを解析・追記せず停止します。Batchではattemptだけが残るため、同じrunの再開も既存のattempt/resolution不一致guardでfail-closedします。新しいrejected cycleではこのreportをheader-onlyへ再初期化します。Fixerの`fixed`、`false_positive`、`cannot_fix`はclaimにすぎずfindingをcloseしません。次のReviewerだけが`resolved`、`invalid`、`unresolved`を決定します。
