...
`review-details.tsv` はvalidated reviewから生成されるFixer向け補助情報で、headerは `severity`, `finding`, `evidence`, `impact`, `required_outcome`, `constraints`, `validation` です。Fixer cycle前にはledger IDと結合した `pending-finding-details.tsv` を生成し、headerは `finding_id`, `severity`, `text`, `evidence`, `impact`, `required_outcome`, `constraints`, `validation` になります。scheduler は `active-finding-details.tsv` を先に、`active-finding.tsv` をcommit markerとして最後に、それぞれatomic replaceします。2つのrenameは同時ではないため、active IDを使う前に両artifactの行数と `finding_id`・`severity`・`text` の完全一致を検証し、途中停止で旧新が混在した場合はFixerを起動せず停止します。active concise row とsource-of-truth docsがnormativeであり、active detailsはpatch設計ではありません。Fixerはrepository上で根拠を確認し、必要な事後条件と制約を満たす最小で安全な修正を判断します。`validation` はguidanceであり、shell inputとして実行しません。

必要なdetails artifactや対応行が欠落・重複・不整合の場合、flowは簡潔なfindingからdetailsを合成せず停止します。各pending IDはrejected-review cycleごとに最大1回だけFixerへ渡され、`fix-resolution.tsv` はそのcycleのclaimを処理順に累積します。各Fixerの前後ではpending 2 artifact、累積resolution、active 2 artifact、生成済みFixer promptがbyte-identicalであることを、Fixerの終了statusにかかわらず確認します。変更時は6 artifactをFixer前の内容へatomic replaceで復元を試み、resolutionを解析・追記せず停止します。directoryへの置換などで復元または復元後の検証ができない場合は、対象artifactと手動復旧用に保持したbackup directoryを表示してhard errorにします。Batchでは復元成功時もattemptだけが残るため、同じrunの再開は既存のattempt/resolution不一致guardでfail-closedします。新しいrejected cycleではこのreportをheader-onlyへ再初期化します。Fixerの`fixed`、`false_positive`、`cannot_fix`はclaimにすぎずfindingをcloseしません。次のReviewerだけが`resolved`、`invalid`、`unresolved`を決定します。
