...
issue flow を回す前の事前診断は次で明示実行します。

```bash
./tools/codex/doctor.sh
```

`doctor.sh` は required command、`gh auth`、consumer config load/validation、`CODEX_FLOW_BASE_REF`、prompt templates、checks command、`.work/` ignore 推奨状態、現在の branch / dirty 状態をまとめて確認します。missing required command は hard failure で、ここには `git`、`gh`、`codex`、`shellcheck` が含まれます。warning のみなら exit `0`、hard failure が 1 件でもあれば non-zero です。
...
issue flow 実行前の standalone 診断です。`FAIL` と `WARN` を分けて出し、`.work/` が ignore されていない場合は推奨 warning のみを出します。
missing required command は hard failure で、`shellcheck` もここに含まれます。
