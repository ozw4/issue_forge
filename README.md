...
`doctor.sh` は required command、`gh auth`、consumer config load/validation、`CODEX_FLOW_BASE_REF`、prompt templates、checks command、`.work/` ignore 推奨状態、現在の branch / dirty 状態をまとめて確認します。warning のみなら exit `0`、hard failure が 1 件でもあれば non-zero です。
