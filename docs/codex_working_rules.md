...
- optional `.issue_forge/shell.sh`
- `AGENTS.md`
- `README.md`
- optional `docs/README.md`
- optional `tools/run_issue.sh`
...
- 各 Fixer の直前に、orchestrator は pending 2 artifact、累積 `fix-resolution.tsv`、active 2 artifact、生成済み Fixer prompt を退避して内容を記録します。Fixer の終了statusにかかわらず直後に照合し、欠落・置換・変更時は6 artifactをFixer前の内容へatomic replaceで復元を試みて失敗させます。directoryへの置換などで復元または復元後の検証ができない場合は、対象artifactと保持したbackup directoryを表示し、manual recovery requiredとしてhard errorにします。Fixer log の解析と claim の追記は照合成功後にだけ行います。Batch resumeでは、改変を起こしたattemptと復元済みresolutionの不一致をhard errorとし、別findingへ進みません。
...
- `tools/consumer/init.sh --scaffold-checks` は opt-in で consumer-owned な最小 starter を `.issue_forge/checks/run_changed.sh` に作れます。既存 file は上書きせず、生成後は consumer が自分たちの repo 向けに編集できます。
- scaffold のためだけに `CODEX_FLOW_CHECKS_COMMAND`、`doctor.sh`、engine-wide required commands を変えません。

## 7. Run convenience scaffold のルール

- `tools/consumer/init.sh --scaffold-run` は opt-in で consumer-owned な `tools/run_issue.sh` と `.issue_forge/shell.sh` を作れます。
- 既存 file は上書きしません。
- `tools/run_issue.sh` は direct vendor entrypoints に委譲し、`start_from_issue.sh` と `run_issue_flow.sh` の責務を再実装しません。
- `.issue_forge/shell.sh` は source 用 snippet です。init が parent shell、shell rc、global PATH を直接変更してはいけません。
- この convenience scaffold がなくても external consumer は `vendor/issue_forge/tools/...` を直接呼べます。
...
## 9. Git hygiene と worktree exclusion
