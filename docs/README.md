...
- external consumer entrypoints は `vendor/issue_forge/tools/consumer/init.sh`、`vendor/issue_forge/tools/issue/create_from_zip.sh`、`vendor/issue_forge/tools/issue/start_from_issue.sh`、`vendor/issue_forge/tools/codex/*.sh` です。`create_from_zip.sh` は `.md` issue zip から GitHub Issue を作る helper で、`.work`、branch、PR は変更しません。local sequential queue は `vendor/issue_forge/tools/codex/run_issue_queue.sh` で、GitHub Actions workflow や Copilot review は使いません。
