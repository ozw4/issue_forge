...
| `vendor/issue_forge/tools/codex/run_issue_queue.sh` | `[options] <issue_number> [issue_number...]`, `--resume`, or `--requeue <issue_number>` | Local-only sequential issue queue: process issues linearly on batch branches, resume a saved local queue, explicitly discard and requeue the current failed Issue, run strict batch review, create one batch PR per batch, and optionally request auto-merge |
...
vendor/issue_forge/tools/codex/run_issue_queue.sh --requeue <issue_number>
```
...
- `--resume` restores the schema-v1 plan and checkpoints from `.work/queue`; it accepts no Issue numbers or other queue options
- `--requeue <issue_number>` destructively discards the current failed Issue attempt and returns it to `queued / context`; it accepts exactly one Issue number, cannot be combined with `--resume`, fresh Issue input, or any other option, and does not resume the queue
...
The queue also passes the batch-local Issue checkpoint paths:

```bash
CODEX_FLOW_PHASE_STATE_FILE=.work/queue/batches/<batch>/issues/<issue>/state.tsv
CODEX_FLOW_ISSUE_HEAD_COMMIT_FILE=.work/queue/batches/<batch>/issues/<issue>/head_commit
```

`CODEX_FLOW_PHASE_STATE_FILE` selects internal queue checkpoint mode and requires both `CODEX_FLOW_ISSUE_HEAD_COMMIT_FILE` and `CODEX_FLOW_SKIP_PUBLISH=1`. These variables remain an internal queue/engine contract; callers resume the complete queue with `run_issue_queue.sh --resume` rather than setting them directly.
...
A fresh queue persists its validated input order and effective options in `.work/queue/plan.tsv`, then initializes queue, batch, and Issue TSV state before creating any batch branch. Queue checkpoints are `initializing`, `batch`, and terminal `done`. Batch checkpoints are `branch`, `issues`, `checks`, `review`, `publish`, optional `merge`, `ack`, and terminal `done`. Issue state moves from `queued / context` to `leased / context`, then through `leased / implementation`, `leased / checks`, `leased / review`, and `leased / commit`. The Issue flow atomically records `committed / archive` only after saving the committed HEAD; the queue records `committed / batch` only after artifact archival, and finally records `acked / done`. Ack means that batch publish succeeded, or that the PR was confirmed merged when `--auto-merge` is enabled; it does not close the GitHub Issue directly.

The schema-v1 state vocabulary is fixed for this local PoC:

| Scope | Status values | Phase values |
| --- | --- | --- |
| queue | `running`, `failed`, `succeeded` | `initializing`, `batch`, `done` |
| batch | `planned`, `running`, `failed`, `succeeded` | `branch`, `issues`, `checks`, `review`, `publish`, `merge`, `ack`, `done` |
| Issue | `queued`, `leased`, `failed`, `committed`, `acked` | `context`, `implementation`, `checks`, `review`, `commit`, `archive`, `batch`, `done` |

Every `plan.tsv` and queue/batch/Issue `state.tsv` starts with `schema_version<TAB>1`. Queue state then stores `status`, `phase`, `current_batch`, and `exit_code`; batch state stores `status`, `phase`, `current_issue`, and `exit_code`; Issue state stores `status`, `phase`, `lease_owner`, and `exit_code`. The Issue lease owner is the deterministic local batch ID. It is local ownership metadata, not a TTL, heartbeat, GitHub label, or distributed lease.

State and scalar checkpoint replacement writes a temporary file in the destination directory and renames it over the destination. This protects readers from partially written individual files, but it is not a multi-file transaction and does not promise `fsync` durability. Reconciliation therefore uses saved artifacts plus the current local Git state for the normal windows between a side effect and its following checkpoint.

Issue `phase` names the next action. In checkpoint mode, `run_issue_flow.sh` validates the saved branch and fixed Issue base commit against the current branch, HEAD, and worktree before dispatching phases. An implementation checkpoint with uncommitted changes advances directly to checks. Checks may rerun. A structurally and semantically valid `accept: yes` review may advance directly to commit. Commit reconciles either uncommitted changes or a clean worktree whose HEAD already advanced from the Issue base commit. A clean worktree at the unchanged base commit is a hard commit error. A phase with both an advanced HEAD and uncommitted changes is rejected rather than inferred.

Checkpoint history rounds continue after the maximum existing round for `checks`, `fix-from-checks`, `review-diff`, `review-untracked`, `review-summary`, `review-raw`, `review`, and `fix-from-review`; implementation remains `round-00`. Existing checkpoint history is not silently overwritten. Check and review fix limits count the corresponding existing fix history, so restarting the process does not reset either budget. Token usage rows are unique by phase, subject, and round.
...
Non-zero queue exit records `failed` on the queue and current batch while preserving their last checkpoint, and records `failed` plus the original exit code on the current Issue when it is still leased. If the Issue flow already reached `committed / archive`, an archive failure preserves that stable commit checkpoint. The queue lock is removed best-effort. Successful queues and batches end as `succeeded / done`; Issues end as `acked / done`. A fresh run checks for a schema-v1 `running` or `failed` queue before clean-worktree and planned-branch availability checks and refuses to overwrite that state. A `succeeded` queue plan may be replaced only when the newly planned branch names and batch directories remain available. This contract does not migrate pre-schema queue artifacts.

Queue `failed` is terminal for that command invocation: no automatic retry or requeue occurs, and an operator must choose `--resume` or the narrowly allowed destructive `--requeue`. Queue `succeeded / done` means every planned batch is `succeeded / done` and every Issue has reached `acked / done`. This is a single-worker, same-machine, same-worktree PoC; it adds no daemon, database, distributed coordination, retry counter, backoff, dead-letter queue, or GitHub Actions workflow.

`--resume` reads all effective options and the ordered Issue list exclusively from `.work/queue/plan.tsv`, accepts only schema version 1, recomputes each batch ID and branch, and requires those values to match the saved batch artifacts. It scans batch states in plan order and resumes the first batch not in `succeeded / done`; queue `current_batch` is validated as an observational checkpoint but is not authoritative over completed batch states. If every batch is complete, it reconciles the queue to `succeeded / done`. Resuming an already successful queue logs a no-op and returns success without rerunning Codex, checks, PR, or merge actions.

Resume is limited to the same local worktree. At a `planned / branch` checkpoint, no saved batch base and no matching local branch means branch processing has not started, so resume performs the normal fetch, saves the resolved base, and creates the branch. A matching local branch without a saved base is inconsistent and fails explicitly. When a saved batch base exists, it must resolve to a commit and be an ancestor of the saved local batch branch; a missing local branch is recreated from that exact saved base only while reconciling the `branch` phase. Later phases fail instead of recovering from a remote branch. A clean worktree may switch to the saved branch. A dirty worktree is accepted only when it is already on that branch, so interrupted implementation or fix changes can be evaluated by the resume-safe phase helper. `.work/current_issue`, `.work/current_branch`, and `.work/base_commit` are restored from the active Issue before Issue dispatch.

The normal reconciliation rules are intentionally narrow: dirty implementation work advances to checks without rerunning implementation; a clean worktree with an Issue commit ahead of its saved base records the current `HEAD` instead of making a duplicate commit; a valid accepted batch review is reused while missing, malformed, or raw-only current review output starts a later history round; existing PR lookup recovers creation completed before metadata persistence; an already merged PR skips a duplicate auto-merge request; a saved branch base can recreate a missing branch only in `branch`; and an already renamed final Issue archive is reused while sibling `.codex.tmp.*` directories are cleaned. Other inconsistent artifact combinations fail explicitly rather than invoking migration or graph-recovery logic.

`--requeue` is a separate destructive operator action for abandoning partial work instead of continuing it with resume. It reads the schema-v1 saved plan and state, computes the target batch from the ordered Issue list, takes the queue lock, performs only requeue processing, and exits successfully without automatically resuming. The operator must explicitly run the two commands in sequence:
...
Requeue is allowed only when the target is in the current plan, has `failed` or `leased` status, is the first unfinished Issue in its batch, and has no later Issue in that batch with progressed state. `committed` and `acked` targets are always rejected. The Issue-local `base_commit` must resolve, and the saved local batch branch must exist and match the branch derived from the plan. This is not a rollback facility for an arbitrary historical Issue. Complete local PR metadata is treated as an already published batch and is rejected rather than rewriting that PR.

With a dirty worktree, requeue requires the current branch to be the target batch branch. With a clean worktree, it may switch to that branch. It then runs `git reset --hard <issue-base-commit>` and the engine's existing exclusion-aware `git clean`, preserving `.work` and the logical `vendor/issue_forge` engine path while discarding partial tracked and untracked repository changes. This destruction is intentional: failed-attempt artifact generations are not retained.

Requeue removes active `.work/codex`, the target Issue's unfinished `codex/` archive and `head_commit`, and the target Issue-local `base_commit` so the next fresh lease writes it from the reset `HEAD`. Earlier committed Issue commits and archives are retained. It preserves batch history but invalidates current batch completion artifacts: `head_commit`, `changed-files.txt`, `checks.log`, `batch-review.txt`, `batch-review.raw.txt`, `pr_number`, and `pr_url`.

After requeue, the Issue is exactly `queued / context` with its batch ID as `lease_owner` and an empty `exit_code`. The batch remains `failed / issues` with `current_issue` set to the target, and the queue remains `failed / batch` with `current_batch` set to the target batch. Only the following explicit `--resume` changes these checkpoints back to running/leased state. Context preparation remains idempotent, so the existing Issue section in `issues.txt` is not duplicated.

Issue traversal skips `acked / done` and validates then skips `committed / batch`. Context checkpoints rerun idempotent context preparation while preserving an existing Issue base commit. Implementation through commit checkpoints restore `.work/codex` and delegate to the Issue checkpoint dispatcher. `committed / archive` reconciles only archival: an existing final `codex/` directory is reused, stale temporary archive directories are removed, and otherwise the current `.work/codex` directory is atomically archived before advancing to `committed / batch`.

Fresh runs reject any existing queue lock. Resume and requeue reject a lock containing a live local PID and atomically replace a stale or malformed lock through the same lock helper. The lock is removed when processing ends. PID lifetime is checked with `kill -0`; hostname, TTL, heartbeat, and cross-host recovery are outside the local PoC contract.
...
It covers `CODEX_RUN_REASONING_EFFORT`, `CODEX_FLOW_SKIP_PUBLISH=1`, one-batch queue processing, schema-v1 queue resume across Issue/batch/publish/merge checkpoints, explicit destructive requeue plus fresh resume of the current failed Issue, lock and branch safety, batch review/fix effort selection, batch PR body generation, fail-fast multi-batch behavior without `--auto-merge`, and the absence of generated GitHub workflow files.
