...
All queue control-plane critical sections use an exclusive kernel `flock` on the stable `.work/queue/control.guard` file. The lock is automatically released when its file descriptor or process exits. Lease acquisition, takeover, release, authoritative state replacement, `current` publication/removal, and each repository/GitHub-mutating batch phase all hold this same guard from durable-owner validation through the mutation or phase. Therefore takeover cannot occur between a fencing check and its state, pointer, lease, repository, or GitHub action.
...
The manifest and minimal planned run are the first resumable boundary and precede lease acquisition, branch creation, or Issue fetch. A losing fresh contender removes only its own minimal preflight run while holding the guard. Resume derives the entire expected batch/Issue graph from that immutable manifest, creates only missing planned entities, and rejects immutable-field mismatches without overwriting valid mutable entities. Repository identity is canonical and credential-free; equivalent SSH and HTTP(S) origin forms resolve to the same host/owner/name identity. Legacy schema-v1 `lease.state` and raw `current` artifacts are explicit unsupported/manual-migration errors.
