...
The queue publishes the strict `.work/queue/current` pointer only after immutable manifest, minimal `run.state`, and complete lease-owner publication. Explicit run-ID resume validates and republishes a nonterminal run; a different nonterminal pointer is a conflict. Completion compare-removes only its own pointer. If `run.state=completed` is durable but `current` or the same-run lease remains, both `--resume current` and `--resume <completed_run_id>` enter an idempotent cleanup-only finalization path. Under `control.guard` it validates manifest/run/repository identity, refuses a live same-run owner or any different-run lease, applies the normal explicit takeover requirement to foreign/unverifiable owners, retires a dead same-run lease to `lease.finalized.<generation>.<token>`, compare-removes only the completed run's pointer, and removes its stale `current_batch`. It invokes no Issue, Git, checks, review, publication, PR, or merge phase. Repeating explicit finalization is successful without creating a new lease generation; `--resume current` with no pointer reports that cleanup is already finalized.
