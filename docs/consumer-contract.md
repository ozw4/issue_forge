...
| `vendor/issue_forge/tools/codex/run_issue_queue.sh` | `[options] <issue_number> [issue_number...]` or `--resume` | Local-only sequential issue queue: process issues linearly on batch branches, resume a saved local queue, run strict batch review, create one batch PR per batch, and optionally request auto-merge |
...
Fresh runs reject any existing queue lock. Resume rejects a lock containing a live local PID and atomically replaces a stale or malformed lock. PID lifetime is checked with `kill -0`; hostname, TTL, heartbeat, and cross-host recovery are outside the local PoC contract.
