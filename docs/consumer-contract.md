...
- for each rejected-review cycle, the scheduler selects one unprocessed pending ID by `blocker`, `major`, then `minor`, preserving `pending-findings.tsv` order within one severity. It builds complete sibling temporary files, atomically replaces `active-finding-details.tsv` first, and atomically replaces `active-finding.tsv` last as the commit marker. The two renames are not one indivisible filesystem operation. Before any active ID is consumed, the engine requires both artifacts to be regular files with valid headers and matching header-plus-zero-or-one-row cardinality, and requires their `finding_id`, `severity`, and `text` fields to match exactly. An interrupted publication can therefore leave an old/new pair, but that pair is a hard error and no Fixer starts
...
- each pending ID receives at most one Fixer attempt in that rejected-review cycle. Fixers run sequentially; multi-agent and parallel Fixer execution are not part of this contract. Immediately before each invocation, the engine backs up and hashes the scheduler-owned `pending-findings.tsv`, `pending-finding-details.tsv`, `fix-resolution.tsv`, `active-finding.tsv`, `active-finding-details.tsv`, and generated Fixer prompt. It verifies all six immediately after the Fixer exits, including a non-zero exit, and before parsing its resolution or appending a claim. A missing, replaced, non-regular, or byte-modified artifact triggers an attempted restore to its pre-Fixer contents with per-file atomic replacement and is then a hard error. If a target was replaced by a directory, or restoration or verification otherwise cannot complete, the engine reports the affected artifact, retains the backup directory for manual recovery, and stops. For Batch, a successfully restored resolution leaves the durable Agent attempt unmatched by a resolution claim, so resume fails closed before another Fixer starts
