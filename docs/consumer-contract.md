...
Resume is limited to the same local worktree. A saved batch base must resolve to a commit and be an ancestor of the saved local batch branch. A missing local branch is recreated from the exact saved base only while reconciling the `branch` phase; later phases fail instead of recovering from a remote branch. A clean worktree may switch to the saved branch. A dirty worktree is accepted only when it is already on that branch, so interrupted implementation or fix changes can be evaluated by the resume-safe phase helper. `.work/current_issue`, `.work/current_branch`, and `.work/base_commit` are restored from the active Issue before Issue dispatch.
