...
`CODEX_FLOW_PHASE_STATE_FILE` selects internal queue checkpoint mode and requires both `CODEX_FLOW_ISSUE_HEAD_COMMIT_FILE` and `CODEX_FLOW_SKIP_PUBLISH=1`. These variables are an internal queue/engine contract; the queue CLI does not yet expose `--resume`.
